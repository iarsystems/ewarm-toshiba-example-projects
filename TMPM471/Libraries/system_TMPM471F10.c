...
#define CG_24M_MUL_6_65625_FPLL (0x006D195DUL<<8U)      /* fPLL = 24MHz * 6.65625   */
...
#define SYSCR_GEAR_Val          (CG_GEAR_FC_1)
#define SYSCR_PRCK_Val          (CG_PRCK_FC_1)
#define SYSCR_MCKSEL_Val        (CG_MCKSEL_2)
...
#define CG_WUPHCR_WUEF_MASK             ((uint32_t)0x00000002)
...
#define EOSC_24M_DIV4_PLLON     (79875000UL)    /* 24.00MHz * 13.3125 / 4 */
...
#define EOSC_24M_DIV8_PLLON     (39937500UL)    /* 24.00MHz * 13.3125 / 8 */
...
#define EOSC_24M_DIV2_PLLON     (159750000UL)   /* 24.00MHz * 13.3125 / 2 */
...
#define SYSCORECLOCK_ACCR_MIN       FIXED_80MHz
#define SYSCORECLOCK_ACCR_MAX       FIXED_120MHz
...
extern uint8_t __FLASH_CODE_ROM_segment_used_start__[];
extern uint8_t __FLASH_CODE_RAM_segment_used_start__[];
extern uint8_t __FLASH_CODE_RAM_segment_used_size__[];
#endif
...
/* System clock is high-speed clock*/
  ...
  #define CORE_TALH (IXTALH)
        ...
        #if ((PLL0SEL_Val & PLL0SEL_MASK) == CG_24M_MUL_6_65625_FPLL)
            #define __CORE_CLK   EOSC_24M_DIV2_PLLON                              /* output clock is 159.75MHz */
        #else                                    /* fc -> reserved */
            #define __CORE_CLK   (0U)
...
{                               /* Get Core Clock Frequency */
    ...
    if (pll0sel && pll0on) {                        /* If PLL enabled  */
        if (CoreClock == EOSC_6M) {      /* If input is 6MHz */
            ...
            if ((TSB_CG->PLL0SEL & PLL0SEL_MASK) == CG_24M_MUL_6_65625_FPLL) {
                CoreClockInput = EOSC_24M_DIV2_PLLON;          /* output clock is 159.75MHz */
            } else {
                CoreClockInput = 0U;    /* fc -> reserved */
            }
...
void SystemInit(void)
{
    ...
    while (TSB_CG->WUPHCR & CG_WUPHCR_WUEF_MASK) {
        /* no processing */
    }                           /* Warm-up */
    TSB_CG->OSCCR |= CG_OSCCR_OSCSEL_SET;
    while (!(TSB_CG->OSCCR & CG_OSCCR_OSCF_MASK)) {
        /* no processing */
    ...
    while((TSB_CG->SYSCR & CG_MCKSELGST_MASK) != (SYSCR_MCKSEL_Val & CG_MCKSEL_MASK) << 16U){
        /* no processing */
    }                         /* Confirm CGSYSCCR<MCKSELGST>="01" */
    while((TSB_CG->SYSCR & CG_MCKSELPST_MASK) != (SYSCR_MCKSEL_Val & CG_MCKSEL_MASK) << 24U){
        /* no processing */
    }                         /* Confirm CGSYSCR<MCKSELPST>="01" */
    while((TSB_CG->SYSCR & CG_GEARST_MASK) != (SYSCR_GEAR_Val & CG_GEAR_MASK) << 16U){
        /* no processing */
    }                         /* Confirm CGSYSCR<GEARST>="00" */
    while((TSB_CG->SYSCR & CG_PRCKST_MASK) != (SYSCR_PRCK_Val & CG_PRCK_MASK) << 16U){
        /* no processing */
    ...
    while (!(TSB_CG->PLL0SEL & CG_PLL0SEL_PLL0ST_MASK)) {
        /* no processing */
...
#endif
static void FCRegSet(volatile uint32_t r0 __attribute__ ((unused)), volatile uint32_t r1 __attribute__ ((unused)), volatile uint32_t r2 __attribute__ ((unused)), volatile uint32_t r3 __attribute__ ((unused)))
{
    __asm("str r1, [r2]");
    __asm("str r0, [r3]");
    return;
...
static void FlashReadClockSet(uint32_t sysclock)
{
    ...
    if(sysclock > SYSCORECLOCK_ACCR_MAX)
    {
        regval = (uint32_t)(FC_ACCR_FCLC_6);
        FCRegSet(regval, kcr_tmp, kcr_addr_tmp, accr_addr_tmp);
        while(TSB_FC->FCACCR != (uint32_t)(FC_ACCR_FCLC_6))
        {
            /* no processing */
        }
    }
    else if((SYSCORECLOCK_ACCR_MAX >= sysclock) && (sysclock > SYSCORECLOCK_ACCR_MIN))
    {
        regval = (uint32_t)(FC_ACCR_FCLC_5);
        FCRegSet(regval, kcr_tmp, kcr_addr_tmp, accr_addr_tmp);
        while(TSB_FC->FCACCR != (uint32_t)(FC_ACCR_FCLC_5))
        {
            /* no processing */
        }
    } else {
        regval = (uint32_t)(FC_ACCR_FCLC_4);
        FCRegSet(regval, kcr_tmp, kcr_addr_tmp, accr_addr_tmp);
        while(TSB_FC->FCACCR != (uint32_t)(FC_ACCR_FCLC_4))
        {
            /* no processing */
        }
    }
