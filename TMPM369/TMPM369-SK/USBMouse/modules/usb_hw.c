...
void USB_HwInit(void)
{
  ...
  /* Enable USB clock */
  CG_USB_Device_Enable(CG_ENABLE);
  CGCKSTP_bit.USBD = 1;
  ...
  USBDPON_IE |= USBDPON;
  /*Pull up pin config*/
  USBDPULLUP_CR |= USBDPULLUP;
  USBDPULLUP_OFF;
  /*HIGH Level interrupt on Power onf*/
  CGIMCGD_bit.EMCGD = 1;
  CGICRCG = 0x05;
  CGIMCGD_bit.INTDEN = 1;
  //
  UDFSINTENB      = 0;
  ...
  NVIC_IntEnable(INT_USBD);
  NVIC_IntPri(INT_USBD,USBD_INT_PRIO);
...
void INT_USBPON_IRQHandler(void)
{
volatile int dummy;
  /*Filter*/
  CGIMCGD_bit.INTDEN = 0;

  for(volatile int k = 0; 1000 >k ; k++);
  if(0 == CGIMCGD_bit.EMCGD)
  {
    if(!(USBDPON_PORT & USBDPON))
    {
      // USB Power Reset
      UDFSPWCTL =  0;
