...
typedef enum __SdSpiCmdInd_t
{
  ...
  _CMD1,      // Activates the card initialization process
  ...
  _CMD25,     // Continuously writes blocks of data until a 全top Tran・
  ...
  _CMD59,     // Turns the CRC option on or off. A ・・in the CRC
              // option bit will turn the option on, a ・・will turn it off

  _ACMD41,    // Activates the card initialization process (Only for SD)
...
#define _CS_H() SdChipSelect(0); SdTransferByte(0xFF)

static uint32_t _SdSendCmd(_SdSpiCmdInd_t ComdInd, uint32_t *pData);
static void _SdCsdImplemet (void);
static _SdState_t _SdInitMedia (void);

static inline _SdState_t _SdReadCardInfo(uint8_t *pData);
static inline _SdState_t _SdRead(uint8_t *pData, uint32_t Add);
static inline _SdState_t _SdWrite(uint8_t *pData, uint32_t Add);
static inline _SdState_t _SdVerify(uint8_t *pData, uint32_t Add);

static DiskCtrlBlk_t _SdDskCtrlBlk;
static uint32_t        _Tnac;
static uint32_t        _Twr;
static uint8_t         _SdSdCsd[16];
static Boolean       _bSdPermWriteProtect;
static Boolean       _bHC;

/*************************************************************************
 * Function Name: _SdSendCmd
 * Parameters: _SdSpiCmdInd_t ComdInd, uint32_t Arg
 *
 * Return: uint32_t
 *
 ...
 *************************************************************************/
static
uint32_t _SdSendCmd(_SdSpiCmdInd_t ComdInd, uint32_t *pData)
{
uint32_t ch = 0xFF;
uint32_t i;
  // Chip Select
  _CS_L();
  // Send command code
  SdTransferByte(_SdCmd[ComdInd].TxData);
  ...
  {
    assert(pData);
    SdTransferByte(*pData >> 24);
    SdTransferByte(*pData >> 16);
    SdTransferByte(*pData >> 8 );
    SdTransferByte(*pData      );
  }
  // Send CRC
  if(_CMD0 == ComdInd)
  {
  ...
  for(i = 9; i && (ch == 0xFF); --i) ch = SdTransferByte(0xFF);
  ...
  case _SdR1b:
    ...
    {
      ch = SdTransferByte(0xFF);
    }
  case _SdR1:
    break;
  case _SdR2:
    ch  = ((uint32_t)ch << 8)     & 0x0000FF00;
    ch |= SdTransferByte(0xFF)  & 0x000000FF;
    break;
  default:
    assert(pData);
    *pData  = ((uint32_t)SdTransferByte(0xFF) << 24) & 0xFF000000;
    *pData |= ((uint32_t)SdTransferByte(0xFF) << 16) & 0x00FF0000;
    *pData |= ((uint32_t)SdTransferByte(0xFF) << 8 ) & 0x0000FF00;
    *pData |=          SdTransferByte(0xFF)        & 0x000000FF;
  }
  return(ch);
}
...
uint32_t i,res, data;
  ...
  _CS_H();
  for(i = 10; i; --i) SdTransferByte(0xFF);
 ...
 *************************************************************************/
static inline
_SdState_t _SdReadCardInfo(uint8_t *pData)
{
uint32_t i;
uint32_t res;
 ...
 *************************************************************************/
inline static
_SdState_t _SdRead(uint8_t *pData, uint32_t Add)
{
uint32_t res,i;
 ...
 *************************************************************************/
inline static
_SdState_t _SdWrite(uint8_t *pData, uint32_t Add)
{
uint32_t res,i;
  ...
  {
    SdTransferByte(0xFF);
    SdTransferByte(_SD_DATA_TOLKEN);
    // Send block
    SdSendBlock(pData,SD_DEF_BLK_SIZE);
    // CRC Send
    SdTransferByte(0xFF);
    SdTransferByte(0xFF);
    res = SdTransferByte(0xFF) & 0x1F;
    if(res != 0x05)
    {
      _CS_H();
      return(_SdCardError);
 ...
 * Parameters: uint8_t *pData, int32_t Add, uint32_t Length
 *
 * Return: _SdState_t
 *
 ...
 *************************************************************************/
inline static
_SdState_t _SdVerify(uint8_t *pData, uint32_t Add)
{
uint32_t res,i;
  res = _SdSendCmd(_CMD17,&Add);
  if(res == _SD_OK)
